...
def get_leaf_cell_multipole(particles, p, cells, leaves, capacity):
    ...
    if cells[p].nleaf >= capacity:
        for c in range(8):
            if cells[p].clist & (1 << c): # if the child exists
                get_leaf_cell_multipole(particles, cells[p].child[c], cells, leaves, capacity)
...
def eval_potential(particles, cells, capacity, theta):
    for i in range(len(particles)):
        evaluate(particles, 0, i, cells, capacity, theta)
