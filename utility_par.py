...
import concurrent.futures as par
...
def eval_potential(particles, cells, sigma, theta):
    # 创建线程池
    with par.ThreadPoolExecutor() as executor:
        # 提交任务给线程池
        futures = [executor.submit(evaluate, particles, 0, i, cells, sigma, theta) for i in range(len(particles))]

        # 获取结果
        for future in par.as_completed(futures):
...
def plot_particles(particles):
    ...
    scatter = ax.scatter([particle.x for particle in particles], 
               ...
               [particle.z for particle in particles],
                s=30, c = u_list, cmap = cmap, norm = color_norm)
    ax.set_xlim3d(0,1)
    ax.set_ylim3d(0,1)
    ax.set_zlim3d(0,1)
    ax.set_xlabel(r'$x$')
    ...
    cbar.ax.set_position([100, 0.1, 0.03, 0.8])
